...
function EnhancedAiInspector:get_mac_from_ip(device_ip)
    ...
    local base_dir = config.base_dir or os.getenv('PROJECT_DIR') or os.getenv('MQTTLIVE_DIR') or '.'
    local mac_helper = base_dir .. '/scripts/get_mac_from_ip.py'
    -- Fallback: check if script is in current directory
    local f = io.open(mac_helper, 'r')
    if not f then
        mac_helper = base_dir .. '/get_mac_from_ip.py'
    else
        f:close()
    end
...
function EnhancedAiInspector:record_detection_stage(mac_address, device_ip, command, threat_level, detection_type)
    ...
    -- Try multiple possible paths - auto-detect from environment
    local base_dir = config.base_dir or os.getenv('PROJECT_DIR') or os.getenv('MQTTLIVE_DIR') or '.'
    local tracker_helper = base_dir .. '/scripts/record_detection.py'
    
    -- Check if file exists, try alternative paths
    local test_cmd = string.format('test -f %s && echo "EXISTS" || echo "NOT_FOUND"', tracker_helper)
    local test_handle = io.popen(test_cmd)
    if test_handle then
        local test_result = test_handle:read("*line")
        test_handle:close()
        if test_result ~= "EXISTS" then
            -- Try alternative path (direct in project root)
            tracker_helper = base_dir .. '/record_detection.py'
            test_cmd = string.format('test -f %s && echo "EXISTS" || echo "NOT_FOUND"', tracker_helper)
            test_handle = io.popen(test_cmd)
            if test_handle then
                test_result = test_handle:read("*line")
                test_handle:close()
                if test_result ~= "EXISTS" then
                    debug_log(string.format("WARNING: record_detection.py not found, detection tracking disabled"))
                    return 1  -- Return stage 1 as fallback
                end
            end
        end
    end
...
function EnhancedAiInspector:block_device_mac(mac_address)
    -- Block device MAC address at gateway level using iptables
    if not mac_address or mac_address == "" then
        debug_log("ERROR: MAC address is empty")
        return false
    end
    
    debug_log(string.format("Attempting to block MAC: %s", mac_address))
    
    -- Try multiple possible paths for block helper - auto-detect from environment
    local base_dir = config.base_dir or os.getenv('PROJECT_DIR') or os.getenv('MQTTLIVE_DIR') or '.'
    local block_helper = base_dir .. '/scripts/block_mac_helper.py'
    
    -- Check if file exists, try alternative paths
    local test_cmd = string.format('test -f %s && echo "EXISTS" || echo "NOT_FOUND"', block_helper)
    local test_handle = io.popen(test_cmd)
    if test_handle then
        local test_result = test_handle:read("*line")
        test_handle:close()
        if test_result ~= "EXISTS" then
            block_helper = base_dir .. '/block_mac_helper.py'
        end
    end
    
    -- Use helper script or direct Python call
    local cmd = string.format('python3 %s "%s" 2>&1', block_helper, mac_address:gsub('"', '\\"'))
    
    local handle = io.popen(cmd)
    if not handle then
        debug_log("ERROR: Failed to execute MAC blocking command")
        return false
    end
    ...
    debug_log(string.format("MAC blocking output: %s", output or "NONE"))
    
    -- Check if blocking succeeded
    if output and (output:match("BLOCKED") or output:match("blocked") or output:match("Blocked") or output:match("IPTABLES")) then
        debug_log(string.format("✅ MAC %s successfully blocked", mac_address))
        return true
    end
    
    debug_log(string.format("❌ MAC blocking failed for %s", mac_address))
    return false
end
...
function EnhancedAiInspector:eval(p, buffer)
    ...
    if is_malicious then
        ...
        local stage = self:record_detection_stage(mac_address, device_ip, command, threat_level, "ai_block")
        debug_log(string.format("Detection Stage: %d", stage))
        
        -- ========================================================================
        -- OPTION A: IMMEDIATE AI DROP + PATTERN TRACKING
        -- If AI says BLOCK → Snort DROPS immediately (via drop())
        -- Detection count still increments for pattern-based escalation
        -- ========================================================================
        
        -- OPTION A: AI BLOCK → IMMEDIATE DROP (not just alert)
        local drop_msg = string.format(
            "[AI BLOCK] Immediate Drop - %s | IP: %s | MAC: %s | Command: %s | Stage: %d",
            detection_method == "both" and "AI BLOCK + Heuristic MAL" or "AI BLOCK",
            device_ip, mac_address, command, stage
        )
        
        debug_log(string.format("AI BLOCK DETECTED: Calling drop() immediately - %s", drop_msg))
        debug_log("🛡️ SNORT DECISION: Dropping packet due to AI BLOCK verdict")
        
        -- CRITICAL: Snort makes the decision and drops the packet
        -- In inline IPS mode, drop() will:
        -- 1. Drop the current packet
        -- 2. Drop subsequent packets in the connection (connection-level drop)
        -- 3. Log to Snort logs automatically
        -- 4. Trigger alert logging to database
        
        debug_log("🛡️ CALLING drop() - This will drop packet AND connection")
        drop({
            msg = drop_msg,
            sid = 1002002,  -- AI Block drop
            gid = 1,
            rev = 1,
        ...
        -- Generate alert for database logging (drop() handles blocking)
        alert({
            msg = drop_msg,
            sid = 1002002,
            gid = 1,
        ...
        debug_log("✅ Snort drop() executed - packet AND connection blocked")
        
        -- Pattern-based escalation: Check if we should escalate to Stage 4 (MAC blocking)
        if stage >= 4 then
            -- ========================================================================
            ...
            if stage >= 4 then
                ...
                -- Packet already dropped above (AI BLOCK), but log Stage 4 escalation
                alert({
                    msg = block_msg,
                    sid = 1002004,  -- Stage 4 escalation alert
                    gid = 1,
                    rev = 1,
                    priority = 1,
                    class = "A Network Trojan was Detected"
                ...
                debug_log("✅ Stage 4: MAC blocking executed (packet already dropped by AI BLOCK)")
            end
        end
